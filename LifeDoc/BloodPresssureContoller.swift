...
class BloodPresssureContoller: UIViewController, WWCalendarTimeSelectorProtocol  {
    ...
    override func viewDidLoad() {
        ...
        self.navItem.title = "Edit Blood Pressure"
        idDiastolic = "0"
        idSystolic = "0"
    ...
    func loadDataSingle(){
        ...
        if (prefs.string(forKey: "singleMeasurement") != nil){
            ...
            for (_, object) in json {
                ...
                if (object["type"].stringValue == "Diastolic") {
                    
                    let newString = object["value"].stringValue.replacingOccurrences(of: ".", with: ",")
                    bpDiastolic.text = newString
                    idDiastolic = object["id"].stringValue
                    bpDiastolic.isEnabled = true
                    deletedList.append(object["id"].stringValue)
                    
                }
                
                if (object["type"].stringValue == "Systolic") {
                    
                    let newString = object["value"].stringValue.replacingOccurrences(of: ".", with: ",")
                    bpSystolic.text = newString
                    idSystolic = object["id"].stringValue
                    bpSystolic.isEnabled = true
                    deletedList.append(object["id"].stringValue)
    ...
    private func prepareCloseButton() {
        ...
        leftNavItem?.action = #selector(BloodPresssureContoller.buttonTapActionClose)
    ...
    private func prepareTime() {
        ...
        bpTime.placeholder = "Time"
        
        bpTime.detailLabel.numberOfLines = 0
        bpTime.resignFirstResponder()
        
        bpTime.delegate = self
        
        bpTime.addTarget(self, action: #selector(myTargetFunction), for: UIControlEvents.touchDown)
        
        let dateToday = Date()
        
        let prefs = UserDefaults.standard
        if (prefs.string(forKey: "singleMeasurement") != nil){
            bpTime.text = editTime
        }else{
    ...
    private func prepareSystolic() {
        
        bpSystolic.placeholder = "Systolic (mmHg)"
        bpSystolic.detail = Constants.err_msg_Systolic
        bpSystolic.isClearIconButtonEnabled = true
        
        bpSystolic.detailLabel.numberOfLines = 0
        
        
        bpSystolic.delegate = self
        
        bpSystolic.addTarget(self,action: #selector(textFieldDidChangesys),for: UIControlEvents.editingDidEnd)
        bpSystolic.addTarget(self,action: #selector(textFieldDidChangesysLength),for: UIControlEvents.editingChanged)
        
    }
    
    func textFieldDidChangesys(textField: UITextField) {
        
        let Str: String = bpSystolic.text!
        if(!Str.isEmpty){
            
            if(!validateValueSys(testStr: Str)){
                
                bpSystolic.isErrorRevealed = true
                bpSystolic.detail = Constants.err_msg_Systolic
            }else if(!validateValueDecimal(testStr: Str)){
                bpSystolic.isErrorRevealed = true
                bpSystolic.detail = Constants.err_msg_two_decimal
                
            }
            else{
                bpSystolic.isErrorRevealed = false
    ...
    func textFieldDidChangesysLength(textField: UITextField) {
        
        
        checkMaxLengthsys(textField: bpSystolic,maxLength: 6)
        
    }
    
    
    func checkMaxLengthsys(textField: UITextField!, maxLength: Int) {
        if (bpSystolic.text!.characters.count > maxLength) {
            bpSystolic.deleteBackward()
        }
    }
    
    private func prepareDiastolic() {
        
        bpDiastolic.placeholder = "Diastolic (mmHg)"
        bpDiastolic.detail = Constants.err_msg_Diastolic
        bpDiastolic.isClearIconButtonEnabled = true
        bpDiastolic.detailLabel.numberOfLines = 0
        bpDiastolic.delegate = self
        
        bpDiastolic.addTarget(self,action: #selector(textFieldDidChangedia),for: UIControlEvents.editingDidEnd)
        bpDiastolic.addTarget(self,action: #selector(textFieldDidChangediaLength),for: UIControlEvents.editingChanged)
        
    }
    
    func textFieldDidChangedia(textField: UITextField) {
        
        let Str: String = bpDiastolic.text!
        if(!Str.isEmpty){
            
            if(!validateValueDia(testStr: Str)){
                
                bpDiastolic.isErrorRevealed = true
                bpDiastolic.detail = Constants.err_msg_Diastolic
            }else if(!validateValueDecimal(testStr: Str)){
                bpDiastolic.isErrorRevealed = true
                bpDiastolic.detail = Constants.err_msg_two_decimal
                
            }
            else{
                bpDiastolic.isErrorRevealed = false
    ...
    func textFieldDidChangediaLength(textField: UITextField) {
        
        
        checkMaxLengthdia(textField: bpDiastolic,maxLength: 6)
        
    }
    
    
    func checkMaxLengthdia(textField: UITextField!, maxLength: Int) {
        if (bpDiastolic.text!.characters.count > maxLength) {
            bpDiastolic.deleteBackward()
        }
    }
    ...
    func buttonTapAction(sender: UIButton!) {
        ...
        let str1: String = bpSystolic.text!
        ...
        if(str1.isEmpty || !validateValueSys(testStr: str1)){
            bpSystolic.isErrorRevealed = true
            bpSystolic.detail = Constants.err_msg_Systolic
            self.bpSystolic.becomeFirstResponder()
            return
        }
        ...
        if(str2.isEmpty || !validateValueDia(testStr: str2)){
            bpDiastolic.isErrorRevealed = true
            bpDiastolic.detail = Constants.err_msg_Diastolic
            self.bpDiastolic.becomeFirstResponder()
            return
        }
        ...
        fields.append("Systolic-" + bpSystolic.text! + "-" + idSystolic);
        fields.append("Diastolic-" + bpDiastolic.text! + "-" + idDiastolic);
    ...
    func validateValueSys(testStr:String) -> Bool {
        
        
        if (testStr.isEmpty) {
            return false
        }
        if (!isValidRangeSys(testStr: testStr)) {
            return false
        }
        return true
    }
    func validateValueDia(testStr:String) -> Bool {
        
        
        if (testStr.isEmpty) {
            return false
        }
        if (!isValidRangeDia(testStr: testStr)) {
            return false
        }
        return true
    }
    ...
    func isValidRangeDia(testStr:String) -> Bool {
        if (!isValidDecimal(testStr: testStr)) {
            return true
        }else{
            ...
            if (valueNew  >= (10*100) && valueNew  <= (250*100)) {
                return true
            }
        }
    ...
    func isValidRangeSys(testStr:String) -> Bool {
        if (!isValidDecimal(testStr: testStr)) {
            return true
        }else{
            let formatter = NumberFormatter()
    ...
    private func deleteRecord() {
        ...
        BloodPresssureContoller.Manager.request(urlString, method: .post, parameters: parameters, encoding: JSONEncoding.default,headers: headers).responseJSON { response in
    ...
    private func sendHealthAssessment() {
        ...
        let strTime: String = bpTime.text!
...
extension BloodPresssureContoller: TextFieldDelegate {
