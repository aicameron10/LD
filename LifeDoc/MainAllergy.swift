...
class MainAllergy: UIViewController, WWCalendarTimeSelectorProtocol, UITableViewDataSource, UITableViewDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate,FileExplorerViewControllerDelegate{
    ...
    private func prepareDate() {
        ...
        bpDate.placeholder = "Date first observed"
    ...
    private func prepareDesc() {
        ...
        desc.placeholder = "Allergy Description"
        desc.detail = Constants.err_msg_allergy
        desc.isClearIconButtonEnabled = true
        
        desc.detailLabel.numberOfLines = 0
        
        
        desc.delegate = self
        
        desc.addTarget(self,action: #selector(textFieldDidChange),for: UIControlEvents.editingDidEnd)
        desc.addTarget(self,action: #selector(textFieldDidChangeLength),for: UIControlEvents.editingChanged)
    ...
    func textFieldDidChangeLength(textField: UITextField) {
        ...
        checkMaxLength(textField: desc,maxLength: 50)
        
    }
    
    
    func checkMaxLength(textField: UITextField!, maxLength: Int) {
        if (desc.text!.characters.count > maxLength) {
            desc.deleteBackward()
        }
    }
...
extension MainAllergy: TextFieldDelegate {
